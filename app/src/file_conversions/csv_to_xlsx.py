...
def convert_all_csv_to_excel(source_folder: str, output_folder: str, workbook_name="", logger_name=""):
    """
    Converts all csv files in a folder to excel workbooks. If a workbook_name is given it adds all sheets to be to a
    single workbook, else it creates a new workbook per csv sheet.
...
def csv_to_excel_workbook(csv_file: str, workbook: Workbook, sheet_name: str):
    """
    Converts a csv file into a excel workbook. Creates one overall workbook and adds a new sheet for each file.
    Credit to https://stackoverflow.com/questions/17684610/python-convert-csv-to-xlsx
    :param csv_file:
    :param workbook:
    :param sheet_name:
    :return:
